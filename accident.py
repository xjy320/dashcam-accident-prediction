...
############### Global Parameters ###############
# path
train_path = os.path.join(os.getcwd(), 'dataset', 'features', 'training')
test_path = os.path.join(os.getcwd(), 'dataset', 'features', 'testing')
demo_path = os.path.join(os.getcwd(), 'dataset', 'features', 'testing')
default_model_path = os.path.join(os.getcwd(), 'model', 'demo_model')
save_path = os.path.join(os.getcwd(), 'model')
video_path = os.path.join(os.getcwd(), 'dataset', 'videos', 'testing', 'positive')
# batch_number
train_num = 126
test_num = 46
...
def parse_args():     # 通过这个方法接受命令行指令，判断运行模式
    """Parse input arguments."""
    ###定义可以接受的命令行参数（有哪些选择/默认值）###
    parser = argparse.ArgumentParser(description='accident_LSTM')
    parser.add_argument('--mode',dest = 'mode',help='train or test',default = 'demo')   # 运行模式
    parser.add_argument('--model',dest = 'model',default= default_model_path)   # 模型路径
    parser.add_argument('--gpu',dest = 'gpu',default= '0')   # 指定 GPU 编号
    ###解析命令行输入的参数 把他们一一对应好###
    args = parser.parse_args()   #创建类似结构体的变量储存这些参数
...
def train():
    ...
    for epoch in range(n_epochs):
         ...
         for batch in n_batchs:
             file_name = '%03d' %batch
             batch_data = np.load(os.path.join(train_path, 'batch_' + file_name + '.npz'))
         ...
         sys.stdout.flush()
         if (epoch+1) %5 == 0:
            save_file = os.path.join(save_path, "model")
            saver.save(sess, save_file, global_step = epoch+1, write_meta_graph=False)
            print("Training")
    ...
    saver.save(sess, os.path.join(save_path, "final_model"), write_meta_graph=False)
...
def test_all(sess,num,path,x,keep,y,loss,lstm_variables,soft_pred):
    ...
    for num_batch in range(1,num+1):
         ...
         test_all_data = np.load(os.path.join(path, 'batch_' + file_name + '.npz'))
...
def vis(model_path):
    ...
    for num_batch in range(1,test_num):
        file_name = '%03d' %num_batch
        all_data = np.load(os.path.join(demo_path, 'batch_' + file_name + '.npz'))
        data = all_data['data']
        ...
        for i in range(len(ID)):
            if labels[i][1] == 1 :
                ...
                counter = 0 
                cap = cv2.VideoCapture(os.path.join(video_path, file_name + '.mp4')) 
                ret, frame = cap.read()
                while(ret):
                    ...
                    for num_box in index:
                        pt1 = (int(new_bboxes[num_box,0]), int(new_bboxes[num_box,1]))
                        pt2 = (int(new_bboxes[num_box,2]), int(new_bboxes[num_box,3]))
                        if now_weight[num_box]/255.0>0.4:
                            cv2.rectangle(frame, pt1, pt2, (0,255,0), 3)
                        else:
                            cv2.rectangle(frame, pt1, pt2, (255,0,0), 2)
                        font = cv2.FONT_HERSHEY_SIMPLEX
                        cv2.putText(frame,str(round(now_weight[num_box]/255.0*10000)/10000), pt1, font, 0.5,(0,0,255),1,cv2.LINE_AA)
